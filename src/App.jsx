...
import GamePhoneView from "./pages/GamePhoneView/GamePhoneView";
...
function App() {
  const [room, setRoom] = useState(Math.floor(Math.random() * 1000000));
  ...
  const handleSetRoomEvent = (e) => {
    setRoom(e);
  };
  const handleSetGameISStartedEvent = (e) => {
    setGameIsStarted(e);
  };
  const handleNewGameEvent = () => {
    console.log("handle new game event");
    createGame();
  };
  const handlesetNicknameEvent = (e) => {
    setNickname(e);
  };
  const handlesetNameIsCreatedEvent = (e) => {
    setNameIsCreated(e);
  };

  // state.game.lastDiscardedCard["red",2];

  //console.log(canDiscard("red",1));
    ...
    <div className="App">
      {isMobile ? (
        ...
        w < h ? (
          ...
          <GamePhoneView room={room} clientId={clientId} />
        ) // Itt a OnPhone verzio kellene
      ) : // laptopon
      !roomIsCreated ? (
        <RoomMaker
          room={room}
